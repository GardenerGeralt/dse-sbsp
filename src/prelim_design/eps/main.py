from src.prelim_design.eps.power_budget import BatteriesMainPower
from src.prelim_design.eps.power_budget import BatteriesBusPower
import src.prelim_sizing.collect_ansys.collect_methods as cm

"""Solar Panel characteristics"""
PV_OP_TEMP = 393.
sample_pv_cell = cm.SolarCell(0.281, 3.018e-3, 1.780e-3, 8.5, PV_OP_TEMP, name="AZUR QJ 4G32C C8.5")
...
power_required_no_bat = 10.14e6  # (placeholder)
...
def panel_battery_sizing():
    ...
    battery = BatteriesMainPower(p_req=power_required_no_bat , T_orb=3600*8.7, t_contact=3600*6, t_eclipse_contact=600, E_spec=392400,
...
def sizing_EPS():
    ...
    battery = BatteriesMainPower(p_req=power_required_no_bat , T_orb=3600*26.93, t_contact=3600*22.29, t_eclipse_contact=60 * 15*60,
...
def sizing_battery(p_req, t_eclipse, E_spec, E_dens, DoD, bat_eff):
    battery = BatteriesBusPower(p_req, t_eclipse, E_spec, E_dens, DoD, bat_eff)
    battery_mass = battery.calc_battery_mass()
    battery_volume = battery.calc_battery_volume()
    battery_charge = battery.calc_battery_energy() /3600
    return battery_volume, battery_mass, battery_charge

def sizing_solar_array():
    panel_area, panel_mass = collect.size(power_required_no_bat, total=True)
    battery = BatteriesBusPower(p_req, t_eclipse, E_spec, E_dens, DoD, bat_eff)
    battery_mass = battery.calc_battery_mass()
    battery_volume = battery.calc_battery_volume()
    battery_charge = battery.calc_battery_energy() / 3600
    return battery_volume, battery_mass, battery_charge
print(sizing_battery(p_req=700, t_eclipse = 0.9558*3600,E_spec=392400, E_dens=4.428e+8, DoD=0.4, bat_eff=0.9))