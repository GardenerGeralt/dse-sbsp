...
def main(pv_cell_df: pd.DataFrame, concentrator_obj: cm.Concentrator, radiator_obj: cm.Radiator, power_req: float):
    pv_cell_objs = [convert_to_object(r) for _, r in pv_cell_df.iterrows()]

    pv_cell_df2 = pd.DataFrame(columns=["PV cell name", "Power density [W/m2]", "Specific power [W/kg]"])
    collectors_df = pd.DataFrame(columns=["PV cell name", "Total area [m2]", "Total mass [kg]"])
    for pv_cell in pv_cell_objs:
        ...
        pv_cell_df2.loc[pv_cell.name, "Power density [W/m2]"] = power_req / total_area
        pv_cell_df2.loc[pv_cell.name, "Specific power [W/kg]"] = power_req / total_mass
        collectors_df.loc[pv_cell.name, "Total area [m2]"] = total_area
        collectors_df.loc[pv_cell.name, "Total mass [kg]"] = total_mass

    # pv_cell_df["Power density [W/m2]"] = pv_cell_df.apply(lambda r: convert_to_object(r).power_dens, axis=1)
    ...
    fig1 = px.scatter(pv_cell_df2, x="Power density [W/m2]", y="Specific power [W/kg]", hover_name="PV cell name")
    fig1.show()

    fig2 = px.scatter(collectors_df, x="Total area [m2]", y="Total mass [kg]", hover_name="PV cell name")
    fig2.show()
...
if __name__ == "__main__":
    ...
    power_required = 1e6
    collect = cm.Collector(sample_pv_cell, solar_sail, radiator)
    print(collect.size(power_required))

    main(pv_cells, fresnel_lens, radiator, power_required)
